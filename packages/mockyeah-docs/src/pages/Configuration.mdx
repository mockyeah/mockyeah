...
- `noWebSocket`: Boolean to disable automatic connection over WebSocket to talk to client-side `@mockyeah/fetch`.
- `record`: Boolean to enable recording on startup.
- `latency`: A default latency for all mock responses. Can override per mock.
