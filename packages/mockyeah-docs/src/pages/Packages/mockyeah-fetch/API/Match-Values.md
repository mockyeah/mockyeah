...
Match values are used both in [mock definitions](Mock-API) and in [expectations](Expectation-API).
...
and can either explicitly return `true` or `false` to indicate pass or fail,
